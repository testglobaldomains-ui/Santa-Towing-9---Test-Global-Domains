      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
        ...
        <Link to="/" className="flex items-center gap-2 group">
          <img 
            ...
            className="h-10 md:h-11 w-auto object-contain bg-white p-1 rounded-xl shadow-sm"
            onError={(e) => {
        ...
        <nav className="hidden lg:flex xl:hidden items-center gap-4 font-medium text-sm">
          ...
          <div className="relative group cursor-pointer">
            ...
            <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 w-48 bg-white text-dark rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 overflow-hidden border border-gray-100 p-1.5 z-50">
              <Link to="/sales" onClick={() => setMobileMenuOpen(false)} className="block px-3.5 py-2.5 hover:bg-slate-50 hover:text-accent transition-colors text-sm font-medium rounded-full">Cars for Sale</Link>
              <Link to="/rental" onClick={() => setMobileMenuOpen(false)} className="block px-3.5 py-2.5 hover:bg-slate-50 hover:text-accent transition-colors text-sm font-medium rounded-full">Rental Fleet</Link>
            </div>
          </div>
                ...
                <AnimatePresence>
                  {mobileTowingOpen && (
                    <motion.div 
                      ...
                      className="overflow-hidden flex flex-col bg-slate-50 rounded-xl my-1 py-1"
                    >
                      {towingServices.map(s => (
                        <Link 
                          key={s.title}
